...
[하자별 식별 단서 — 표면·형태로 구분]
- 균열(구조 A-02/마감 A-03): 선형으로 갈라진 틈. 길고 방향성 있고 깊으면 구조,
...
- 몰딩 구분 규칙: 트림(몰딩)은 방향으로 판단 — '수평' 바닥 몰딩은 걸레받이(C-05),
  '수직' 문·창 옆 세로 몰딩(문선·창선)은 창틀·문틀류로 본다. 수직 몰딩의 도장 벗겨짐·
  얼룩은 창틀·문틀 도장 불량(E-02), 찍힘·긁힘은 찍힘·스크래치(C-04)로 분류.
- 도색 얼룩·붓자국(C-03): 페인트 면의 얼룩·번짐·붓자국·색차.
- 찍힘·스크래치(C-04): 표면이 긁히거나 패인 선형 흠.
- 유리 스크래치·파손(E-01): 창유리 표면 긁힘·금(유리에서만).
- 창틀·문틀 도장 불량(E-02): 프레임(창틀/문틀)의 도장 벗겨짐·얼룩.
- 코킹 누락·불량(B-03): 실리콘 코킹 빠짐·들뜸·곰팡이·변색(창호·욕실 등).
- 방수·누수 흔적(B-04): 물 얼룩·번짐·곰팡이·들뜸(천장·벽·바닥).
- 수직·수평도(A-01)/틀 직각도(A-04): 벽·천장·창호·문틀이 기울거나 직각 안 맞음.

[판정 규칙]
1. 위 목록의 class_name 에 정확히 해당하는 하자만 보고합니다. 목록 외 항목은 무시합니다.
2. [마킹 무시] 점검자가 표시한 테이프(특히 초록·형광 테이프)·화살표·동그라미·스티커·
   손글씨·형광펜 자국은 하자가 아닙니다. 절대 그 마킹 자체를 검출하지 말고, 색·모양·위치를
   하자 단서로도 쓰지 마세요 (실제 현장에는 마킹이 없는 게 정상입니다).
   ★ 중요: 테이프가 붙어 있다고 거기에 반드시 하자가 있는 것은 아닙니다. 마킹은 없는 셈 치고,
   그 자리의 '실제 표면'을 독립적으로 보고 표면 자체에 하자가 보일 때만 보고하세요.
   테이프가 없는 다른 부위도 동일한 기준으로 똑같이 살핍니다(테이프 구간에만 집중 금지).
